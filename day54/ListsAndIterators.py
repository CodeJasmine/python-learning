...
# 列表a不是迭代器类型
a = [1, 3, 5, 7]
# 要想成为迭代器，需要经过内置函数iter包装
a_iter = iter(a)
# 验证a_iter是否Iterator迭代器
...
# 再次遍历a、a_iter 就会不同，a_iter没有打印出任何信息
...
for i in a_iter:
    print(i, end=' ')
# 要想迭代器a_iter重新指向a的表头，需要重新创建一个新的迭代a_iter_copy
a_iter_copy = iter(a)
...
# 原因：等迭代到最后一个元素后一个元素后，再执行next，会触发StopIteration异常
